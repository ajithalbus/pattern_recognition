...
def rgb2cat(img,order=0):
    #print (img[0,0,0])
    tmp=np.empty((img.shape[0],img.shape[0]))
    ...
    return tmp#np.reshape(tmp,(img.shape[0],img.shape[1],1))
    ...
    if colour=='cat':
        for i in range(image.shape[0]):
            for j in range(image.shape[1]):
                new_image[i,j]=splitter(image[i,j],order)
...
def evd(img,colour,order=-1,per=""):
    ...
    #while n<=val.shape[0]:
    for n in [10,25,50,100,125,150,175,200,225,250,251,252,253,254,255]:
        tmp=np.copy(val)
        print 'N='+str(n)
        ...
        error=np.linalg.norm(img-np.clip(np.real(rec),0,255))/np.linalg.norm(img)
    ...
    savefig('./result/sq/graph'+colour+"_"+per+'.png')
    #show()        

img=read_image('./dataset/sq.jpg')
