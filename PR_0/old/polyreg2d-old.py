...
def polyfit_deg2(x0,x1,y,k=0):
    x=np.column_stack((x0,x1))
    X=[]
    for x0i,x1i in x:
        line =[[pow(x0i,d)*pow(x1i,e) for d in range(3)] for e in range(3)]
...
def polyvalxy(coef,x0,x1):
    val=0
    for p in [2,1,0]:
        for k in [2,1,0]:
             val=val+pow(x0,k)*pow(x1,p)*coef[8-(p*3+k)]
    return val
...
def operations(x0,x1,y,l): #l=lamda
    ...
    print 'training'
    best_coef=polyfit_deg2(x0_train,x1_train,y_train)
    
    #validating 
    print 'validating...'
    y_valid_result=np.array([polyvalxy(best_coef,k,p) for k,p in np.column_stack((x0_valid,x1_valid))])
    ...
    y_train_result=np.array([polyvalxy(best_coef,k,p) for k,p in np.column_stack((x0_train,x1_train))])
    ...
    #testing 
    print 'testing...'
    y_test_result=np.array([polyvalxy(best_coef,k,p) for k,p in np.column_stack((x0_test,x1_test))])
    ...
    j=0
    for i in x0_test:
        print i,'\t',x1_test[j],'\t',y_test[j],'\t',polyvalxy(best_coef,i,x1_test[j])
        j=j+1
