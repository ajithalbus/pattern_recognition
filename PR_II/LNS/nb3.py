...
def plotr(mu,sig,color):
    ...
    #ax.set_xlim(mu[0]-5*sig[0][0]**0.5, mu[0]+5*sig[0][0]**0.5)
    #ax.set_ylim(mu[1]-5*sig[1][1]**0.5, mu[1]+5*sig[1][1]**0.5)
...
#working block , remove commentation later
...
N = 60
X = np.linspace(avg_mu[0]-3*avg_sig[0][0]**0.5, avg_mu[0]+3*avg_sig[0][0]**0.5, N)
Y = np.linspace(avg_mu[1]-3*avg_sig[1][1]**0.5, avg_mu[1]+3*avg_sig[1][1]**0.5, N)
...
np.savetxt('../op_data/NLS/nls-case5-fa.txt',c_fa)
np.savetxt('../op_data/NLS/nls-case5-miss.txt',c_miss)
np.savetxt('../op_data/NLS/nls-case5-tp.txt',c_tp)
